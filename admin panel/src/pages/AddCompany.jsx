...
import { createCompany, fetchCompanies } from '../redux/slices/companySlice';
...
export default function AddCompany() {
  ...
  const validateField = (value) => {
    ...
    if (!value || value.trim() === '') {
      ...
      const isDuplicate = companies.some(b => b.name.toLowerCase().trim() === value.toLowerCase().trim());
      if (isDuplicate) {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(createCompany({ name, status })).then((res) => {
      if (!res.error) {
        Swal.fire('Success!', `company "${name}" added successfully!`, 'success');
        navigate('/companies');
      } else {
        Swal.fire('Error!', res.payload || 'Failed to add company.', 'error');
      }
    });
  };
    ...
    <div className="page-content">
      <div className="page-header d-flex align-items-center gap-3">
        ...
        <div>
          ...
          <p className="page-subtitle">Define a new company category</p>
        </div>
      </div>
                ...
                <div className="d-flex gap-2 mt-4">
                  <button type="submit" className="btn-accent px-5 py-3 flex-grow-1" disabled={loading}>
                    {loading ? (
                      ...
                      <><i className="bi bi-check2-circle me-2" /> Save Company</>
                    )}
                  </button>
