...
import { createFormula } from '../../redux/slices/formulaSlice';
...
export default function AddFormula() {
  ...
  // Pre-fill initial column when raw materials load
  useEffect(() => {
    if (rawMaterialsList && rawMaterialsList.length > 0 && formData.columns.length === 1 && formData.columns[0].rawMaterials.length === 1 && formData.columns[0].rawMaterials[0].rawMaterialId === '') {
      const clearGloss = rawMaterialsList.find(rm => rm.name.toUpperCase() === 'CLEAR GLOSS' || rm.name.toUpperCase() === 'CLEAR-GLOSS');
      const ecoCoatGloss = rawMaterialsList.find(rm => rm.name.toUpperCase() === 'ECO COAT GLOSS' || rm.name.toUpperCase() === 'ECO-COAT GLOSS');
      
      setFormData(prev => {
        const newCols = [...prev.columns];
        newCols[0].rawMaterials = [
          { rawMaterialId: clearGloss ? clearGloss._id : '', quantity: '' },
          { rawMaterialId: ecoCoatGloss ? ecoCoatGloss._id : '', quantity: '' }
        ];
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    dispatch(createFormula(payload)).then((res) => {
      setLoading(false);
      if (!res.error) {
        Swal.fire('Success!', 'Formula saved successfully!', 'success');
        navigate('/formulas');
      } else {
        Swal.fire('Error', res.payload || 'Failed to save formula', 'error');
      }
    });
  };

  // Filtering dependent dropdowns based on previous selections
  ...
  return (
    <div className="page-content">
      <div className="page-header d-flex align-items-center gap-3 user-form-page-header">
        ...
        <div>
          <h1 className="page-title">Formula</h1>
          <p className="page-subtitle">Select Company, Brand, Bottle Name, and Variant</p>
        </div>
      </div>
                ...
                <div className="d-flex flex-column flex-md-row gap-2 mt-5 user-form-actions">
                  <button type="submit" className="btn-accent px-5 py-3 flex-grow-1" disabled={loading}>
                    {loading ? (
                      ...
                      <><i className="bi bi-check2-circle me-2" /> Save Formula</>
                    )}
                  </button>
                  <button type="button" onClick={() => navigate('/')} className="btn-ghost px-5 py-3">
                    Cancel
                  </button>
                </div>
              </form>
