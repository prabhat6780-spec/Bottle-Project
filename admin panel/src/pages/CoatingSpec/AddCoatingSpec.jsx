...
import { createCoatingSpec, fetchCoatingSpecs } from '../../redux/slices/coatingSpecSlice';
...
export default function AddCoatingSpec() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { brands } = useSelector((state) => state.brands);
  const { companies } = useSelector((state) => state.companies);
  const { items: coatingTypes } = useSelector((state) => state.coatingType);
  const { coatingSpecs, loading } = useSelector((state) => state.coatingSpecs);
  ...
  // When returning from Add Variant, once the variant is loaded in Redux,
  // auto-fill the coating shade from it if the form doesn't already have one
  useEffect(() => {
    if (location.state?.newVariantId && variants.length > 0) {
      const newVariant = variants.find(v => v._id === location.state.newVariantId);
      if (newVariant?.coatingShade) {
        setFormData(prev => ({
          ...prev,
          coatingShade: prev.coatingShade || newVariant.coatingShade
        }));
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Duplicate check: same brand + bottleName + variant
    ...
    try {
      ...
      const res = await API.post('/bottle-spec', data, { headers: { 'Content-Type': 'multipart/form-data' } });
      if (res.data) {
        Swal.fire('Success!', `Coating Spec "${formData.bottleName}" added!`, 'success');
        navigate('/coating-specs');
      }
    } catch (err) {
      Swal.fire('Error!', err.response?.data?.message || 'Failed to add coating spec.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page-content">
      <div className="page-header d-flex align-items-center gap-3 user-form-page-header">
        ...
        <div>
          <h1 className="page-title">Add Coating Spec</h1>
          <p className="page-subtitle">Configure coating specification details</p>
        </div>
      </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="row g-4">
                  ...
                  <div className="col-md-6">
                    ...
                    <select className={`form-select custom-input-field ${errors.coatingTypeId ? 'is-invalid' : ''}`} name="coatingTypeId" value={formData.coatingTypeId} onChange={handleChange} onBlur={handleBlur} style={{ borderRadius: 12 }}>
                      ...
                      {coatingTypes.filter(t => t.status).map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
                    </select>
                ...
                <div className="d-flex flex-column flex-md-row gap-2 mt-5 user-form-actions">
                  <button type="submit" className="btn-accent px-5 py-3 flex-grow-1" disabled={saving}>
                    {saving ? <><span className="spinner-border spinner-border-sm me-2"></span>Saving...</> : <><i className="bi bi-check2-circle me-2" />Save Coating Spec</>}
                  </button>
