...
import { updateShift } from '../../redux/slices/shiftSlice';
...
export default function EditShift() {
  ...
  const validateField = (value) => {
    ...
    if (!value || value.trim() === '') {
      ...
      const isDuplicate = shifts.some(b => 
        b._id !== id && 
  ...
  useEffect(() => {
    const shift = shifts.find(b => b._id === id);
    if (shift) {
      setName(shift.name);
      setStatus(shift.status === true ? 'active' : 'inactive');
    }
  }, [id, shifts]);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateShift({ id, formData: { name, status } })).then((res) => {
      if (!res.error) {
        Swal.fire('Updated!', 'shift updated successfully!', 'success');
        navigate('/shifts');
      } else {
        Swal.fire('Error!', res.payload || 'Failed to update shift.', 'error');
      }
    });
  };
    ...
    <div className="page-content">
      <div className="page-header d-flex align-items-center gap-3 user-form-page-header">
        ...
        <div>
          ...
          <p className="page-subtitle">Update shift details</p>
        </div>
      </div>
                ...
                <div className="d-flex gap-2 mt-4 user-form-actions">
                  <button type="submit" className="btn-accent px-5 py-3 flex-grow-1" disabled={loading}>
                    {loading ? (
                      ...
                      <><i className="bi bi-check2-circle me-2" /> Update Shift</>
                    )}
                  </button>
