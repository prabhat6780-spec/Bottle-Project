...
import { createSgLabel } from '../../redux/slices/sgLabelSlice';
...
export default function AddSgLabel() {
  ...
  // Pre-fill if navigated from a master record
  useEffect(() => {
    if (bottleSpecs.length > 0 && variants.length > 0) {
      ...
      if (initBottleId && initVariantId && !formData.bottleId) {
  ...
  }, [bottleSpecs, variants, searchParams]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!res.error) {
      Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'SG Label saved successfully',
        timer: 1500,
        showConfirmButton: false
      });
      navigate('/sg-labels');
    } else {
      Swal.fire('Error', res.payload || 'Failed to save SG Label', 'error');
    }
  };

  // Filter available bottle specs to show active ones
  const availableSpecs = bottleSpecs.filter(s => s.status);

  // Filter variants based on selected bottle spec
  ...
  return (
    <div className="page-content">
      <div className="page-header d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3 user-form-page-header">
        ...
        <div>
          <h1 className="page-title">Add SG Label</h1>
          <p className="page-subtitle">Create a new printing or coating label</p>
        </div>
      </div>
                ...
                <div className="d-flex flex-column flex-md-row gap-2 mt-5 user-form-actions">
                  <button type="submit" className="btn-accent px-5 py-3 flex-grow-1" disabled={loading}>
                    {loading ? (
                      ...
                      <><i className="bi bi-check2-circle me-2" /> Save SG Label</>
                    )}
                  </button>
